...
public:
	...
	void hash_and_map(G1&,char *, int IDL);
	void random(Big&);
	void rankey(Big&);
	void random(G1&);
...
// creates a smaller table
	...
	Big hash_to_group(char *, int IDL);
	Big hash_to_Big(char *, int IDL);
	char* hash_to_Char(char *, int sl);
...
// parameters: number of pairings n, pointers to pair of G1 elements
	...
	void add_to_hash(char* s, int length);
