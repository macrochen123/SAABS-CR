...
void PFC::add_to_hash(char* s, int length)
{
	int i;
	for ( i = 0; i < length; i++)
	{
		   shs256_process(&SH,s[i]);
	}
}
...
void PFC::hash_and_map(G1& w,char *ID, int IDL)
{
    Big x0=H1(ID, IDL);
...
Big PFC::hash_to_group(char *ID, int IDL)
{
	Big m=H1(ID, IDL);
	return m%(*ord);
}
Big PFC::hash_to_Big(char *ID, int IDL)
{
	Big m=H1(ID, IDL);
	return m;
}
char* PFC::hash_to_Char(char *string, int sl)
{
	char s[HASH_LEN];
	int i; 
	sha256 sh;
